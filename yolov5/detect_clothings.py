...
from utils.plots import Annotator, AnnotatorClothings, colors, save_one_box, get_top_n_colors, crop_frame
from utils.torch_utils import select_device, smart_inference_mode

CROPPED_CLOTHINGS_IMAGES_SAVE_PATH = Path(r'D:\pd2_app\cropped_clothings_images')
MAIN_PATH = os.getcwd()
...
def run(
        ...
        save_colors_and_timestamps=True, # ADDED FOR SAVING CLOTHINGS FOUND WITH THEIR TIMESTAMPS AND COLORS
        ...
        clothings_to_detect_and_colors=None,
        cleanup_data=None,
        progress_data=None,
        unix_timestamp_datetime_created=None,
        is_frame_in_nightvision_list=None,
        is_clothing_only=True
):
    source = str(source)
    ...
    frame_counter = 0
    total_frame_count = int(cv2.VideoCapture(source).get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    if unix_timestamp_datetime_created == None:
        try:
        ...
        input_video_creation_timestamp_unix = unix_timestamp_datetime_created - 2 # 2 seconds is the offset from the time the media was created


    for path, im, im0s, vid_cap, s in dataset:
        frame_counter += 1

        # is_frame_in_nightvision = is_frame_in_nightvision_list[frame_counter - 1]
        # if not is_frame_in_nightvision:
        with dt[0]:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            annotator = AnnotatorClothings(im0, line_width=line_thickness, example=str(names))
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if is_frame_in_nightvision_list == None:
                        is_frame_in_nightvision = (im0s[:,:,0]==im0s[:,:,1]).all()
                    else:
                        is_frame_in_nightvision = is_frame_in_nightvision_list[frame_counter - 1]
                    
                    
                    if (save_img or save_crop or view_img):  # Add bbox to image
                        if not is_frame_in_nightvision: # If not in night vision - add bbox to clothings if matched with desired color
                            c = int(cls)  # integer class
                            label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                            cropped_frame = crop_frame(xyxy=xyxy, im=imc, BGR=False)
                            top_n_colors = get_top_n_colors(im=cropped_frame)
                            top_color_without_duplicate = list(set(top_n_colors)) # Remove if the palette will be returned
                            desired_color = clothings_to_detect_and_colors[label.split(' ')[0]].replace('_', ' ').title()
                            
                            if desired_color in top_color_without_duplicate: # Change back to top_n_colors if list of palette will be returned
                                # print(f'{desired_color} ?-> {top_n_colors}')
                                to_append_to_clothings_text_file = True
                                annotator.box_label(box=xyxy, label=label, color=colors(c, True))
                            else:
                                to_append_to_clothings_text_file = False

                            # timestamp_from_frame_number = get_timestamp_from_frame(frame_counter, input_video_fps)
                            ...
                            if (namesc_and_timestamp not in stored_seconds_to_avoid_ms_duplicated_cropped_frames) and (to_append_to_clothings_text_file):
                                stored_seconds_to_avoid_ms_duplicated_cropped_frames.append(namesc_and_timestamp)
                                # add_to_filename = timestamp_from_frame_number.replace(':', '-')
                                # top_n_colors = get_top_n_colors(im=cropped_frame)
                                # top_n_colors = get_top_n_colors(xyxy, imc, file=CROPPED_CLOTHINGS_IMAGES_SAVE_PATH / names[c] / f'{names[c]},{add_to_filename}.jpg', BGR=True)
                                if names[c] in UPPER_CLOTHINGS:
                                    clothings_subcategory = 'upper_clothing'
                                elif names[c] in LOWER_CLOTHINGS:
                                    clothings_subcategory = 'lower_clothing'
                                with open(os.path.join(TEMP_PATH, 'file_clothings.txt'), 'a') as f:
                                    f.write(f'{[clothings_subcategory, names[c], top_color_without_duplicate], unix_timestamp_from_frame_rounded}\n')
                                    # f.write(f'{[clothings_subcategory, names[c], top_n_colors], unix_timestamp_from_frame_rounded}\n') # Uncomment and use if going back to palette list
                                # print(n, top_n_colors, seconds_component_of_timestamp)
                        else: # If in nightvision mode, add bbox to the clothing detected regardless of the color
                            c = int(cls)  # integer class
                            label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                            annotator.box_label(box=xyxy, label=label, color=colors(c, True))
                            top_n_colors = ['n/a']
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    # im0 = cv2.putText(im0, f'{utc_timestamp_frame} {unix_timestamp_frame_video}', (50,70), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                    ###
                    if is_frame_in_nightvision_list == None: # is_frame_in_nightvision_list is None, it means that we are using clothing od model only
                        unix_timestamp_frame_video = input_video_creation_timestamp_unix + (vid_cap.get(cv2.CAP_PROP_POS_MSEC)/1000)
                        utc_timestamp_frame = datetime.fromtimestamp(unix_timestamp_frame_video)
        ...
        if is_clothing_only:
            ...
            progress_bar_value_points = np.round(np.arange(0, 1.01, 0.5/progress_data['num_of_input_video_filepaths']), 2)
            progress_bar_min_value = progress_bar_value_points[progress_data['zero_indexed_nth_video_filepath'] + 1] * 100
            progress_bar_max_value = progress_bar_value_points[progress_data['zero_indexed_nth_video_filepath'] + 2] * 100
            progress_value_count = map_value_to_range(frame_counter, from_min=0, from_max=total_frame_count, to_min=progress_bar_min_value, to_max=progress_bar_max_value)
            progress_data['progress_callback'].emit(progress_value_count)
        ...
        # print(progress_bar_value_points)
        # print(progress_bar_min_value, progress_bar_max_value)
...
def parse_opt():
    ...
    parser.add_argument('--save-colors-and-timestamps', action='store_true', help='save timestamps where clothings found and their colors')
